...
function Save() {
  ...
  const buttonLabel = useMemo(() => {
    if (viewType === 'creation') return 'Lưu';
    return 'Lưu';
  }, [viewType]);
...
function SaveThenRequestSign() {
  ...
  const handleClick: MouseEventHandler<HTMLButtonElement> = useCallback(
    (event) => {
      event?.stopPropagation?.();
      dispatchSubmit?.(MUTATE_ACTION.DRAFT_THEN_ASSIGN);
    },
    [dispatchSubmit]
  );
  const buttonLabel = useMemo(() => {
    if (viewType === 'creation') return 'Lưu và Trình ký';
    return 'Trình ký';
  }, [viewType]);
  const $Icon = useMemo(() => {
    if (viewType === 'creation') return <NoteAddIcon />;
  ...
  return (
    <GridItemAction>
      <ButtonPositive startIcon={$Icon} endIcon={$EndIcon} onClick={handleClick}>
        {buttonLabel}
      </ButtonPositive>
    </GridItemAction>
  );
