...
function ButtonClear() {
  ...
  const hasValues = useMemo(() => Array.isArray(appliedField) && appliedField.length > 0, [appliedField]);
    ...
    return (
      <FormGridItem disabledXs sx={{ ...fieldSx, flex: 'unset !important' }}>
        <ButtonCommon
          ...
          startIcon={<ClearIcon />}
        >
          {appliedField.length > 0 ? `Xóa bộ lọc (${appliedField.length})` : `Xóa bộ lọc`}
        </ButtonCommon>
      </FormGridItem>
    );
  }, [hasValues, handleClick, isSubmitted, appliedField]);
  return $Button;
}
