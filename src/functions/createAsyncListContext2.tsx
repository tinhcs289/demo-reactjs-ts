...
export type AsyncListSelectable<T extends RowData = RowData> = {
  selectable: boolean;
  selectedItems: T[];
  selectedItemIds: any[];
  isCheckAll: boolean;
...
export default function createAsyncListContext<T extends RowData>(defaultState?: Partial<AsyncListState<T>>) {
  ...
  function useDefaultPropInit(field: string, value?: number | string | boolean) {
    const setState = useAsyncListSetter();
    const state = useAsyncListGetter((s) => get(s, field));
