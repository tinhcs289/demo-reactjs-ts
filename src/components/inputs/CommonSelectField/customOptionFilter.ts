...
const _removeVietnameseAccentMarks = (value?: string) =>
  typeof value === 'string'
    ? _mergeFormater(
...
const _isContains = (value?: string, subValue?: string) =>
  typeof value !== 'string' || typeof subValue !== 'string' ? false : `${value}`.indexOf(`${subValue}`) > -1;

const _toLowerCaseTrim = (text?: string) => (!!text ? text.toLowerCase().trim() : '');

const _execute = (fn: (...agrs: any[]) => boolean, ...agrs: any[]) => {
